...
- **Known, deliberate scope limit**: the Staffline portal was not reachable this round (no authenticated browser session), so "Staffline submitted hours" is not a real figure anywhere in this release -- Gmail's approval emails carry a Timesheet ID and date range but never a submitted-hours number. Calendar's expected hours are compared directly against the payslip's paid hours instead; Staffline's own role in the reconciliation is existence, date range and job classification. Detail-level portal scraping remains a real, named follow-up, not silently dropped.
...
- **Discovered, not introduced by this release**: real payslip processing (`processPayslip()`, the step that turns a stored PDF into structured actual-pay figures) currently fails for every real payslip with a missing Docs API OAuth scope (`https://www.googleapis.com/auth/documents`) -- see `docs/VERSION.md`. All 48 real payslips already in the Payslip Register have an empty Import Status, suggesting this has likely never completed successfully in this deployment. This blocks the whole Payroll Centre's PDF-parsing feature, not just Staffline, and is left for the user to fix deliberately (it needs an OAuth re-authorization), not changed automatically.
