...
Replaces manual, per-page "sync now" clicking as the app's primary freshness mechanism with a central orchestrator. Post-roadmap initiative: changes how the existing v3 features stay fresh, not what they do. Full audit basis in `docs/v3.2-unified-sync-audit.md`; full release record in `docs/VERSION.md`.

- **New `Backend/Sync/` module**: a task registry (`SyncConfig.js`) covering all 6 real external syncs (Calendar, Staffline Gmail, Payslip Gmail + processing, Annual Leave Gmail, Monzo transactions, Monzo pots) plus Staffline Reconciliation's zero-write recompute, each with a stable ID, dependency list, freshness TTL and startup/scheduled eligibility. `SyncService.js` orchestrates dependency-ordered runs, wrapping each real, already-proven sync function (never rewriting it) behind a pure result mapper that classifies its real return shape onto one common status. `SyncController.js` adds two-tier concurrency safety: a short-lived Properties-backed run-lock (a second concurrent request joins the active run instead of duplicating it) plus each task's own existing document lock. `SyncStateRepository.js` persists per-task health to a new "Sync Status" sheet, upserted by task ID -- a failed attempt never erases the last-known-good timestamp. [PR #36](https://github.com/deanstewartdev-hub/Pay-Tracker/pull/36).
- **Real source wiring** [PR #37](https://github.com/deanstewartdev-hub/Pay-Tracker/pull/37): wired all 7 tasks to their real, already-proven functions. Monzo's OAuth has no headless reconnect path, so a disconnected Monzo reports `Manual` (never `Failed`) so the startup screen never reads it as a broken sync. Reconciliation is confirmed zero-write and never auto-creates Action Centre items -- it only reports a real needs-review count.
  - **Real bug caught by the deliberate live-wiring verification** (not the mocked automated suite, by design): `mapPayslipResult_` read `scan.recordsCreated`, but `PayTrackerPayslipImportService.scanGmail()`'s real return shape uses `payslipsImported`. Beyond a cosmetic `"undefined new"` message, this made a real batch of processed payslips incorrectly report "Already current" instead of "Updated". Fixed, with a regression test using the real shape.
- **Startup screen** [PR #38](https://github.com/deanstewartdev-hub/Pay-Tracker/pull/38): evolves the existing `#app-loading` screen (not a new splash) to show real per-task progress -- a fast freshness read renders immediately, then each row updates to its real final status/message once the sync completes. Dashboard (the one page that loads unconditionally at startup) now waits for the startup sync before its first load, without reintroducing PR #26's "every workspace fires its RPC at startup" problem for any other page. The header refresh button now runs a forced full re-check through the same engine and progress UI ("Refresh Everything"); every existing page-specific sync button is untouched.
  - **Real UX gap found live**: a stale Calendar sync genuinely took 220-262 seconds against real production data -- the original design only offered a "Continue to Pay Tracker" fallback on a hard failure, which would have trapped a routine stale-Calendar startup at the loading screen for minutes. Fixed by racing the sync against a 7-second timer; past that, the same Continue button appears regardless of failure vs. just-slow, and the eventual real result surfaces as a background toast.
  - **Real bug caught live**: a bare `<`/`>`/`<=`/`>=` comparison in the new relative-time helpers got HTML-entity-escaped by `HtmlService`'s template evaluation, silently killing the entire `AppController` script block (a previously-documented recurring platform pitfall for this codebase -- see `docs/CONTRIBUTING.md`). Confirmed via the Apps Script executions log (the new sync RPCs never fired) and fixed with `Math.sign()`-based comparisons.
- **Background schedule**: `SyncAutomationService.js` mirrors the existing `CalendarAutomationService.js` precedent -- three time-driven triggers (06:00 full, 12:00 lightweight subset, 18:00 full), `Europe/London`, idempotent install/remove, status always derived live from `ScriptApp.getProjectTriggers()`.
- **Real production-data-corruption bug found and fixed** [PR #39](https://github.com/deanstewartdev-hub/Pay-Tracker/pull/39): `runUnifiedSyncTests()` writes real rows to the live "Sync Status" sheet keyed by real task IDs to exercise the engine end-to-end -- unlike other v3 suites' domain data, that sheet is active state the live engine reads to decide whether to skip a real sync as "still fresh", not harmless clutter. Caught at the worst possible moment: the final pre-promotion gate-check test run left `MONZO_POTS` showing fake "Injected test success." data instead of its real last-sync time, right as v3.2 went live in production. Fixed by snapshotting every real task's row before the suite runs and restoring it exactly (byte-for-byte, including timestamps) in a `finally` block, mirroring the trigger-state backup/restore convention already used elsewhere in the same file. Live-verified with a before/after snapshot around a full test run: identical on both sides.
- **Legacy trigger retired**: the pre-existing standalone `runAutomaticPayTrackerCalendarSync` trigger became redundant once the new engine's own Calendar task existed (both call the same `PayTrackerCalendarService.sync()`), and was observed actively failing/timing out in production from lock contention with a concurrent real sync request. Disabled once the new engine's own triggers were installed and verified.
- **Verification**: static validation clean; `runAllPayTrackerTests()` 9/9 suites, 218/218 checks (up from 8/8 and 155/155 at v3.1.1); a dedicated isolated test deployment smoke-tested clean; multiple real, live full syncs against production data confirmed no unexplained writes (Gmail sources' own `duplicatesSkipped` counts, Monzo's own existing dedup logic); a post-production smoke test (real startup progress, real forced re-sync) before the new triggers were installed; one further manual production sync run and inspected through the released code before leaving the triggers enabled.
- **Production promotion**: promoted to the existing production deployment (same deployment ID, URL unchanged) on 2026-08-31 from `main` commit `979f3ad` (previously version @137, v3.1.1, `main` commit `1b66fef`; now version @140). See `docs/VERSION.md` for the full deployment record and rollback plan.
